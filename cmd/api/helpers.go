...
/*********************** VERSION 2 of readJSON() ****************************************/
func (app *application) readJSON(w http.ResponseWriter, r *http.Request, dst any) error {
	// using http.MaxBytesReader() to limit the size of the request body to 1 MB.
	maxBytes := 1_048_576
	r.Body = http.MaxBytesReader(w, r.Body, int64(maxBytes))
	// Initialize the json.Decoder(), and call the DisallowUnknownFields() method on it
	// before decoding. This means that if the JSON from the client now includes any field which
	// cannot be mapped to the target destination, the decoder will return an error instead
	// of just ignoring the field.
	...
	if err != nil {
		...
		// destination, then Decode() will now return an error message in the format "json: unknown
		...
		// the size limit of 1MB and we return a clear error message.
	...
	// Call Decode() again, using a pointer to an anonymous struct as the
	// destination. If the requested body only contained a single JSON value
